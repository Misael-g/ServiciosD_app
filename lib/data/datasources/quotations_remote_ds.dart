...
class QuotationsRemoteDataSource {
  ...
  Future<QuotationModel> createQuotation({
    ...
    try {
      ...
      // Actualizar estado de la solicitud a 'quotation_sent' si está en 'pending'
      await _supabase.rpc('update_request_status_on_quotation', params: {
  ...
  /// Aceptar cotización (Cliente) - CORREGIDO
  Future<QuotationModel> acceptQuotation(String quotationId) async {
    try {
      print('📤 [QUOTATIONS_DS] Aceptando cotización: $quotationId');

      // Obtener la cotización primero
      ...
      print('   Service Request ID: ${quotation['service_request_id']}');
      print('   Technician ID: ${quotation['technician_id']}');

      // Usar una transacción RPC para hacer todo de una vez
      await _supabase.rpc('accept_quotation', params: {
        'quotation_id': quotationId,
      });

      print('✅ [QUOTATIONS_DS] Cotización aceptada exitosamente');
      ...
      print('❌ [QUOTATIONS_DS] Error al aceptar cotización:');
      print('   Error: $e');
      print('   StackTrace: $stackTrace');
  ...
  Future<QuotationModel> rejectQuotation(String quotationId) async {
    try {
      print('📤 [QUOTATIONS_DS] Rechazando cotización: $quotationId');
      ...
      print('✅ [QUOTATIONS_DS] Cotización rechazada');

      return QuotationModel.fromJson(response);
    } catch (e, stackTrace) {
      print('❌ [QUOTATIONS_DS] Error al rechazar cotización:');
      print('   Error: $e');
      print('   StackTrace: $stackTrace');
